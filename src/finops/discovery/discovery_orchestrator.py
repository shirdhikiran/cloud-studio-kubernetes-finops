...
"""Fixed discovery orchestrator for data collection only."""
...
class DiscoveryOrchestrator:
    """Discovery orchestrator for data collection only."""
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.discovery_client = DiscoveryClient(self.azure_config)
            self.discovery_client.set_clients(
                aks_client=azure_clients["aks"],
                cost_client=azure_clients["cost"], 
                monitor_client=azure_clients["monitor"],
                k8s_client=k8s_client
            )
            await self.discovery_client.connect()
            ...
            self.logger.info("Discovery Orchestrator initialized successfully")
            
        except Exception as e:
            self.logger.error("Failed to initialize Discovery Orchestrator", error=str(e))
